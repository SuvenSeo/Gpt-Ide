...
void test("writes and reads a UTF-8 file atomically", async () => {
...
void test("rejects a stale expected hash", async () => {
  const root = await mkdtemp(path.join(tmpdir(), "gpt-ide-file-"));
  const service = new FileService(await PathPolicy.create(root));
...
void test("enforces the configured file-size limit", async () => {
  const root = await mkdtemp(path.join(tmpdir(), "gpt-ide-file-"));
